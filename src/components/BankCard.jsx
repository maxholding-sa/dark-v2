...
export default function BankCard({ bank, compact = false }) {
  return (
    <Card
      className={
        compact
          ? "relative flex h-40 w-full cursor-default flex-col overflow-hidden border border-white/10 bg-white/5 p-0 shadow-sm md:aspect-square md:h-auto"
          : "relative flex aspect-square w-full cursor-default flex-col overflow-hidden border border-white/10 bg-white/5 p-0 shadow-sm"
      }
    >
      {bank.logoImage ? (
        <>
